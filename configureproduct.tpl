{include file="orderforms/standard_cart/common.tpl"}
<style>
    select #inputConfigOption3 > option:first-child{
        margin:30px !important;
    }

    /* Custom CSS to change placeholder color */
    #order-standard_cart .summary-container {
    margin: 0;
    padding: 10px;
    min-height: 100px;
    border-radius: 0px;
    background-color: #f8f8f8;
    font-size: 1.1em;
    line-height: 1.7em;
    }

    #order-standard_cart label, #order-standard_cart p.domain-renewal-desc {
    margin-right: 0px !important;
    font-weight: 400 !important;
    font-size: 14px !important;
    }


    .custom-placeholder::placeholder {
        color: #cecdcd !important;
        /* Change this to the color you want */
    }

    .input-group-text {
        border-radius: 0.5rem 0 0 .5rem !important;
        border: 0px !important;
        color: #f8f9fa !important;
        background-color: #369 !important;
    }

    .form-control {
        border-radius: 0 0.5rem .5rem 0 !important;
        color: #404040 !important;
        background-color: #33669917 !important;
        border: 0px !important;
    }

    .head {
        border-radius: 1.5rem 0.5rem 0 0 !important;
        background-color: #3288de !important;
        width: 30% !important;
    }

    .down {
        border-radius: 0 0.5rem 1.5rem 0.5rem !important;
    }

    .form-control {
        height: 45px !important;

    }

    #order-standard_cart .order-summary .product-name {
        display: none !important;
        font-weight: 700 !important;
        font-size: 1.2em !important;
    }

    #order-standard_cart .order-summary .product-group {
        margin: 0 0 5px 0 !important;
        display: none !important;
        font-style: italic !important;
    }

    .summary-container {
        width: 100% !important;
    }

    .summary-container>div.clearfix:nth-child(3) {
        background-color: #2685e561;
        font-size: 130% !important;
        border-radius: 30px !important;
        margin: 10px 0px !important;
        padding: 0.7rem 1.5rem !important;
        color: #333333;
    }

    .summary-container>div.clearfix:nth-child(n+4) {
        padding: 0.3rem 0.6rem !important;
    }



    #order-standard_cart .order-summary .summary-totals {
        margin: 50px 0 !important;
        padding: 15px 20px !important;
        border-top: 1px solid #ccc !important;
        border-bottom: 1px solid #ccc !important;
        font-weight: 600 !important;
    }

    #order-standard_cart .order-summary .total-due-today .amt {
        font-size: 160% !important;
        padding: 8px 0px !important;
    }

    .mygap {
        height: 100px !important;
    }

    .total-due-today {
        padding: 50px 0px;
    }
</style>

<script>
    var _localLang = {
        'addToCart': '{$LANG.orderForm.addToCart|escape}',
        'addedToCartRemove': '{$LANG.orderForm.addedToCartRemove|escape}'
    }
</script>

<div id="order-standard_cart">
    <div class="row">
        <div class="cart-body" style="width: 100%;">
            <!-- Title -->
            <div class="header-lined">
                <p class="h4" style="color: #058;">{$LANG.orderconfigure}</p>
            </div>
            <form id="frmConfigureProduct">
                <input type="hidden" name="configure" value="true" />
                <input type="hidden" name="i" value="{$i}" />
                <div class="row">
                    <!-- Configs -->
                    <div class="col-12 ">
                        <p>{$LANG.orderForm.configureDesiredOptions}</p>

                        <!-- Product -->
                        <div class="mt-4">
                            <div class="bg-primary text-light col-12 h5 m-0 px-3 py-3 head">
                                <p class="p-0 m-0">{$productinfo.name}</p>
                            </div>
                            <div class="col-12 bg-white text-dark h6 m-0 px-3 py-3 down">
                                <p>{$productinfo.description}</p>

                                {if $pricing.type eq "recurring"}
                                <div class="field-container my-5">
                                    <div class="form-group">
                                        <div class="input-group">
                                            <div class="input-group-prepend">
                                                <label class="input-group-text m-0 px-4 bg-primary text-light"
                                                    for="inputBillingcycle">{$LANG.cartchoosecycle}</label>
                                            </div>
                                            <select name="billingcycle" id="inputBillingcycle"
                                                class="form-control select-inline custom-select"
                                                onchange="updateConfigurableOptions({$i}, this.value); return false">
                                                {if $pricing.monthly}
                                                <option value="monthly" {if $billingcycle eq "monthly" } selected{/if}>
                                                    {$pricing.monthly}
                                                </option>
                                                {/if}
                                                {if $pricing.quarterly}
                                                <option value="quarterly" {if $billingcycle eq "quarterly" }
                                                    selected{/if}>
                                                    {$pricing.quarterly}
                                                </option>
                                                {/if}
                                                {if $pricing.semiannually}
                                                <option value="semiannually" {if $billingcycle eq "semiannually" }
                                                    selected{/if}>
                                                    {$pricing.semiannually}
                                                </option>
                                                {/if}
                                                {if $pricing.annually}
                                                <option value="annually" {if $billingcycle eq "annually" }
                                                    selected{/if}>
                                                    {$pricing.annually}
                                                </option>
                                                {/if}
                                                {if $pricing.biennially}
                                                <option value="biennially" {if $billingcycle eq "biennially" }
                                                    selected{/if}>
                                                    {$pricing.biennially}
                                                </option>
                                                {/if}
                                                {if $pricing.triennially}
                                                <option value="triennially" {if $billingcycle eq "triennially" }
                                                    selected{/if}>
                                                    {$pricing.triennially}
                                                </option>
                                                {/if}
                                            </select>
                                        </div>
                                    </div>
                                </div>
                                {/if}

                            </div>
                        </div>

                        <div class="alert alert-danger w-hidden" role="alert" id="containerProductValidationErrors">
                            <p>{$LANG.orderForm.correctErrors}:</p>
                            <ul id="containerProductValidationErrorsList"></ul>
                        </div>

                        <!-- Metrics  -->
                        {if count($metrics) > 0}
                        <div>
                            <div class="mygap"></div>
                            <div class="h5 bg-primary py-3 px-3 text-light rounded-lg">
                                <span>{$LANG.metrics.title}</span>
                            </div>

                            <p>{$LANG.metrics.explanation}</p>

                            <ul>
                                {foreach $metrics as $metric}
                                <li>
                                    {$metric.displayName}
                                    -
                                    {if count($metric.pricing) > 1}
                                    {$LANG.metrics.startingFrom} {$metric.lowestPrice} / {if
                                    $metric.unitName}{$metric.unitName}{else}{$LANG.metrics.unit}{/if}
                                    <button type="button" class="btn btn-default btn-sm" data-toggle="modal"
                                        data-target="#modalMetricPricing-{$metric.systemName}">
                                        {$LANG.metrics.viewPricing}
                                    </button>
                                    {elseif count($metric.pricing) == 1}
                                    {$metric.lowestPrice} / {if
                                    $metric.unitName}{$metric.unitName}{else}{$LANG.metrics.unit}{/if}
                                    {if $metric.includedQuantity > 0} ({$metric.includedQuantity}
                                    {$LANG.metrics.includedNotCounted}){/if}
                                    {/if}
                                    {include file="$template/usagebillingpricing.tpl"}
                                </li>
                                {/foreach}
                            </ul>
                            <br>
                        </div>
                        {/if}

                        <!-- Configure Server -->
                        {if $productinfo.type eq "server"}
                        <div class="mygap"></div>
                        <div>
                            <div class="bg-primary text-light rounded-lg h5 m-0 p-0 py-3 px-3 head">
                                <span>{$LANG.cartconfigserver}</span>
                            </div>

                            <div class="col-12 bg-white text-dark h6 m-0 px-3 py-3 pt-5 down">
                                <div class="row">
                                    <div class="col-12">
                                        <div class="input-group flex-nowrap mb-4">
                                            <div class="input-group-prepend">
                                                <label class="input-group-text" style="width: 130px;"
                                                    for="inputHostname"
                                                    id="inputHostname">{$LANG.serverhostname}</label>
                                            </div>
                                            <input type="text" name="hostname" class="form-control custom-placeholder"
                                                id="inputHostname" value="{$server.hostname}"
                                                placeholder="servername.example.com">
                                        </div>
                                    </div>
                                    <div class="col-12">
                                        <div class="input-group flex-nowrap mb-4">
                                            <div class="input-group-prepend">
                                                <label class="input-group-text" style="width: 130px;" for="inputRootpw"
                                                    id="inputRootpw">{$LANG.serverrootpw}</label>
                                            </div>
                                            <input type="password" name="rootpw" class="form-control custom-placeholder"
                                                id="inputRootpw" value="{$server.rootpw}" placeholder="*****">
                                        </div>
                                    </div>
                                </div>


                                <!-- NS -->
                                <div class="row mt-5">
                                    <div class="col-sm-6">
                                        <div class="input-group flex-nowrap mb-4">
                                            <div class="input-group-prepend">
                                                <label class="input-group-text" style="width: 130px;"
                                                    for="inputNs1prefix"
                                                    id="inputNs1prefix">{$LANG.serverns1prefix}</label>
                                            </div>
                                            <input type="text" name="ns1prefix" class="form-control custom-placeholder"
                                                id="inputNs1prefix" value="{$server.ns1prefix}"
                                                placeholder="{$LANG.serverns1prefixexample}">
                                        </div>
                                    </div>
                                    <div class="col-sm-6">
                                        <div class="input-group flex-nowrap mb-4">
                                            <div class="input-group-prepend">
                                                <label class="input-group-text" style="width: 130px;"
                                                    for="inputNs2prefix"
                                                    id="inputNs2prefix">{$LANG.serverns2prefix}</label>
                                            </div>
                                            <input type="text" name="ns2prefix" class="form-control custom-placeholder"
                                                id="inputNs2prefix" value="{$server.ns2prefix}"
                                                placeholder="{$LANG.serverns2prefixexample}">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        {/if}

                        <!-- Configure Options -->
                        {if $configurableoptions}
                        <div class="mygap"></div>
                        <div>
                            <div class="bg-primary text-light rounded-lg h5 m-0 p-0 py-3 px-3 head">
                                <span>{$LANG.orderconfigpackage}</span>
                            </div>
                            <div class="col-12 bg-white text-dark h6 m-0 px-3 py-3 pt-5 down">
                                <div class="row">
                                    <div class="col-12">
                                        <div class="product-configurable-options" id="productConfigurableOptions">
                                            <div class="row">
                                                {foreach $configurableoptions as $num => $configoption}
                                                    {if $configoption.optiontype eq 1}
                                                    <div class="col-12">
                                                        <div class="input-group flex-nowrap mb-4">
                                                            <div class="input-group-prepend">
                                                                <label class="input-group-text" style="width: 160px;"
                                                                    for="inputConfigOption{$configoption.id}">{$configoption.optionname}</label>
                                                            </div>
                                                            <select name="configoption[{$configoption.id}]" id="inputConfigOption{$configoption.id}" class="form-control">
                                                                {foreach key=num2 item=options from=$configoption.options}
                                                                    <option value="{$options.id}" {if $configoption.selectedvalue eq $options.id} selected="selected" {/if}>
                                                                        {$options.name}
                                                                    </option>
                                                                {/foreach}
                                                            </select>
                                                        </div>
                                                    </div>
                                                    {elseif $configoption.optiontype eq 2}
                                                    <div class="col-sm-6">
                                                        <div class="form-group">
                                                            <label
                                                                for="inputConfigOption{$configoption.id}">{$configoption.optionname}</label>
                                                            {foreach key=num2 item=options from=$configoption.options}
                                                            <br />
                                                            <label>
                                                                <input type="radio" name="configoption[{$configoption.id}]"
                                                                    value="{$options.id}" {if $configoption.selectedvalue eq
                                                                    $options.id} checked="checked" {/if} />
                                                                {if $options.name}
                                                                {$options.name}
                                                                {else}
                                                                {$LANG.enable}
                                                                {/if}
                                                            </label>
                                                            {/foreach}
                                                        </div>
                                                    </div>
                                                    {elseif $configoption.optiontype eq 3}
                                                    <div class="col-sm-6">
                                                        <div class="form-group">
                                                            <label
                                                                for="inputConfigOption{$configoption.id}">{$configoption.optionname}</label>
                                                            <br />
                                                            <label>
                                                                <input type="checkbox"
                                                                    name="configoption[{$configoption.id}]"
                                                                    id="inputConfigOption{$configoption.id}" value="1" {if
                                                                    $configoption.selectedqty} checked{/if} />
                                                                {if $configoption.options.0.name}
                                                                {$configoption.options.0.name}
                                                                {else}
                                                                {$LANG.enable}
                                                                {/if}
                                                            </label>
                                                        </div>
                                                    </div>
                                                    {elseif $configoption.optiontype eq 4}
                                                    <div class="col-12 px-4 mt-5">
                                                        <div class="form-group">
                                                            <label for="inputConfigOption{$configoption.id}">{$configoption.optionname}</label>
                                                            {if $configoption.qtymaximum}
                                                                {if !$rangesliderincluded}
                                                                    <script type="text/javascript" src="{$BASE_PATH_JS}/ion.rangeSlider.min.js"></script>
                                                                    <link href="{$BASE_PATH_CSS}/ion.rangeSlider.css" rel="stylesheet">
                                                                    <link href="{$BASE_PATH_CSS}/ion.rangeSlider.skinModern.css" rel="stylesheet">
                                                                    {assign var='rangesliderincluded' value=true}
                                                                {/if}
                                                                <input type="text" name="configoption[{$configoption.id}]" value="{if $configoption.selectedqty}{$configoption.selectedqty}{else}{$configoption.qtyminimum}{/if}" id="inputConfigOption{$configoption.id}" class="form-control" />
                                                                <script>
                                                                    var sliderTimeoutId = null;
                                                                    var sliderRangeDifference = {$configoption.qtymaximum} - {$configoption.qtyminimum};
                                                                    // The largest size that looks nice on most screens.
                                                                    var sliderStepThreshold = 25;
                                                                    // Check if there are too many to display individually.
                                                                    var setLargerMarkers = sliderRangeDifference > sliderStepThreshold;

                                                                    jQuery("#inputConfigOption{$configoption.id}").ionRangeSlider({
                                                                        min: {$configoption.qtyminimum},
                                                                        max: {$configoption.qtymaximum},
                                                                        grid: true,
                                                                        grid_snap: setLargerMarkers ? false : true,
                                                                        onChange: function() {
                                                                            if (sliderTimeoutId) {
                                                                                clearTimeout(sliderTimeoutId);
                                                                            }

                                                                            sliderTimeoutId = setTimeout(function() {
                                                                                sliderTimeoutId = null;
                                                                                recalctotals();
                                                                            }, 250);
                                                                        }
                                                                    });
                                                                </script>
                                                            {else}
                                                                <div>
                                                                    <input type="number" name="configoption[{$configoption.id}]" value="{if $configoption.selectedqty}{$configoption.selectedqty}{else}{$configoption.qtyminimum}{/if}" id="inputConfigOption{$configoption.id}" min="{$configoption.qtyminimum}" onchange="recalctotals()" onkeyup="recalctotals()" class="form-control form-control-qty" />
                                                                    <span class="form-control-static form-control-static-inline">
                                                                            x {$configoption.options.0.name}
                                                                    </span>
                                                                </div>
                                                            {/if}
                                                        </div>
                                                    </div>
                                                    {/if}
                                                    {if $num % 2 != 0}
                                                </div>
                                                <div class="row">
                                                    {/if}
                                                {/foreach}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        {/if}

                        <!-- customfields  -->
                        {if $customfields}
                        <div class="mygap"></div>
                        <div class="">
                            <div class="bg-primary text-light rounded-lg h5 m-0 p-0 py-3 px-3 head">
                                <span>{$LANG.orderadditionalrequiredinfo}<br><i><small>{lang
                                            key='orderForm.requiredField'}</small></i></span>
                            </div>

                            <div class="col-12 bg-white text-dark h6 m-0 px-3 py-3 pt-5 down">
                                <div class="row">
                                    <div class="col-12">
                                        <div class="field-container">
                                            {foreach $customfields as $customfield}
                                            <div class="form-group">
                                                <label for="customfield{$customfield.id}">{$customfield.name}
                                                    {$customfield.required}</label>
                                                {$customfield.input}
                                                {if $customfield.description}
                                                <span class="field-help-text">
                                                    {$customfield.description}
                                                </span>
                                                {/if}
                                            </div>
                                            {/foreach}
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        {/if}


                        <!-- addons -->
                        {if $addons || count($addonsPromoOutput) > 0}
                        <div class="mygap"></div>
                        <div id="productAddonsContainer">
                            <div class="sub-heading">
                                <span class="primary-bg-color">{$LANG.cartavailableaddons}</span>
                            </div>

                            {foreach $addonsPromoOutput as $output}
                            <div>
                                {$output}
                            </div>
                            {/foreach}

                            <div class="row addon-products">
                                {foreach $addons as $addon}
                                <div class="col-sm-{if count($addons) > 1}6{else}12{/if}">
                                    <div
                                        class="panel card panel-default panel-addon{if $addon.status} panel-addon-selected{/if}">
                                        <div class="panel-body card-body">
                                            <label>
                                                <input type="checkbox" name="addons[{$addon.id}]" {if $addon.status}
                                                    checked{/if} />
                                                {$addon.name}
                                            </label><br />
                                            {$addon.description}
                                        </div>
                                        <div class="panel-price">
                                            {$addon.pricing}
                                        </div>
                                        <div class="panel-add">
                                            <i class="fas fa-plus"></i>
                                            {$LANG.addtocart}
                                        </div>
                                    </div>
                                </div>
                                {/foreach}
                            </div>
                        </div>
                        {/if}
                    </div>

                    <!-- Order Summery -->
                    <div class="col-12 m-0 p-0">
                        <div class="mygap"></div>
                        <div class="col-12" id="scrollingPanelContainer">
                            <div id="orderSummary">
                                <div class="order-summary d-flex flex-row"
                                    style="background-color:rgba(0, 0, 0, 0); border-bottom: 0;">
                                    <div class="loader" id="orderSummaryLoader">
                                        <i class="fas fa-fw fa-sync fa-spin"></i>
                                    </div>
                                    <div class="col-md-4 m-0 p-0 bg-primary text-light rounded-lg h5 m-0 p-0 py-3 px-3 head d-none d-md-block"
                                        style="border-radius: 1.5rem 0 0 1.5rem !important;">
                                        <p class="pt-4">{$LANG.ordersummary}</p>
                                    </div>
                                    <div class="summary-container down px-3" id="producttotal"
                                        style="background-color: #ffffff; padding: 20px !important;">
                                    </div>
                                </div>
                                <div class="mygap"></div>
                                <div class="text-center row">
                                    <button type="submit" id="btnCompleteProductConfig"
                                        class="col-12 btn btn-primary btn-lg">
                                        {$LANG.continue}
                                        <i class="fas fa-arrow-circle-right"></i>
                                    </button>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </form>
        </div>
    </div>
</div>

<script>recalctotals();</script>

{include file="orderforms/standard_cart/recommendations-modal.tpl"}